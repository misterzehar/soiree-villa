...
import { submitLieuOffer, updateLieuOffer } from './actions'
...
export default async function LieuBriefDetailPage({ params }: { params: Promise<{ briefId: string }> }) {
  const { briefId } = await params

  const authClient = await createSupabaseServerClient()
  const { data: { user } } = await authClient.auth.getUser()
  if (!user) redirect(`/connexion?redirect=/lieu/demandes/${briefId}`)
  ...
  const { data: briefData } = await supabase
    .from('briefs')
    .select('id, title, description, city, event_date, expires_at, budget_estimate_cents, status, organizer_id, organizers(display_name)')
    .eq('id', briefId)
    .eq('target_type', 'lieu')
    .single()
  ...
  const canSend = !isClosed && !isExpired && lieu.is_approved
  const canEdit = canSend && offer?.status === 'submitted'

  async function handleSubmit(formData: FormData) {
    'use server'
    await submitLieuOffer(briefId, formData)
  }

  async function handleUpdate(formData: FormData) {
    'use server'
    await updateLieuOffer(offer!.id, briefId, formData)
  }
    ...
    <main className="min-h-screen bg-bg">
      <div className="max-w-md mx-auto px-4 pt-6 pb-20">
        ...
        <div className="bg-surface rounded-2xl p-5 shadow-sm mb-5">
          ...
          {organizerName && (
            <p className="text-text-muted text-xs">Organisateur : {organizerName}</p>
          )}
            ...
            <form action={handleSubmit} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  rows={4}
                  placeholder="Décrivez ce que vous proposez, vos disponibilités, vos atouts…"
