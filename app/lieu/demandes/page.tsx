...
export default async function LieuDemandesPage() {
  const authClient = await createSupabaseServerClient()
  const { data: { user } } = await authClient.auth.getUser()
  if (!user) redirect('/connexion?redirect=/lieu/demandes')
  ...
  if (!lieu) redirect('/lieu/inscription')
  if (!lieu.is_approved) {
    return (
      <main className="min-h-screen bg-bg">
        <div className="max-w-md mx-auto px-4 pt-8 pb-20">
          <Link href="/lieu" className="inline-flex items-center gap-1.5 text-text-muted hover:text-text text-sm mb-6 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Espace lieu
          </Link>
          <div className="bg-warning/10 border border-warning/25 rounded-2xl p-5">
            <p className="font-semibold text-warning text-sm">Lieu en attente de validation</p>
            <p className="text-text-muted text-sm mt-1">Vous recevrez les appels d'offres par email dès validation de votre fiche.</p>
          </div>
        </div>
      </main>
    )
  ...
  // Briefs ouverts matchant ville + type lieu
  const { data: briefsData } = await supabase
    .from('briefs')
    .select('id, title, description, city, event_date, budget_estimate_cents, expires_at, created_at')
    .eq('target_type', 'lieu')
    .eq('city', lieu.city)
    .eq('status', 'open')
  ...
  const briefs = (briefsData ?? []) as Pick<Brief, 'id' | 'title' | 'description' | 'city' | 'event_date' | 'budget_estimate_cents' | 'expires_at' | 'created_at'>[]

  // Offres soumises par ce lieu
  ...
  for (const o of myOffersData ?? []) {
    myOffersByBrief[o.brief_id] = o.status as OfferStatus
  }
    ...
    <main className="min-h-screen bg-bg">
      <div className="max-w-md mx-auto px-4 pt-6 pb-20">
        ...
        <p className="text-text-muted text-sm mb-6">
          Appels d&apos;offres ouverts pour les lieux à {lieu.city}.
        </p>

        {briefs.length === 0 ? (
          <div className="bg-surface rounded-2xl p-8 text-center">
            <FileText className="w-8 h-8 text-text-muted mx-auto mb-3" />
            <p className="text-text-muted text-sm">Aucune demande en cours pour votre ville.</p>
