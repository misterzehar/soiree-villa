...
export default async function ResultPage() {
  ...
  const axisLabels: Record<string, { positive: string; negative: string; label: string }> = {
  ...
  const axisValues: Record<string, { active: boolean; activeLabel: string }> = {
    energy:    { active: profile.axes.energy    === 'extra',       activeLabel: profile.axes.energy    === 'extra'       ? axisLabels.energy.positive    : axisLabels.energy.negative    },
    structure: { active: profile.axes.structure === 'structured',  activeLabel: profile.axes.structure === 'structured'  ? axisLabels.structure.positive  : axisLabels.structure.negative  },
    depth:     { active: profile.axes.depth     === 'deep',        activeLabel: profile.axes.depth     === 'deep'        ? axisLabels.depth.positive      : axisLabels.depth.negative      },
    sociality: { active: profile.axes.sociality === 'large',       activeLabel: profile.axes.sociality === 'large'       ? axisLabels.sociality.positive  : axisLabels.sociality.negative  },
  }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-surface rounded-2xl shadow-sm p-5 mb-6">
          ...
          <div className="flex flex-col gap-3">
            {(Object.entries(axisLabels) as [string, { positive: string; negative: string; label: string }][]).map(
              ([key, { label, positive, negative }]) => (
                <div key={key}>
                  <div className="flex justify-between text-xs text-text-muted mb-1">
                    <span>{label}</span>
                    <span className="font-semibold text-primary">
                      {axisValues[key].activeLabel}
                    </span>
                  </div>
                  <div className="h-2 bg-border rounded-full overflow-hidden">
                    <div
                      className={[
                        ...
                        axisValues[key].active ? 'w-full' : 'w-1/4',
                      ].join(' ')}
                    />
                  </div>
